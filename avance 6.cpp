...
bool correoYaExiste(const char* correo, int* indiceExistente) {
    for (int i = 0; i < totalContactos; i++) {
        if (strcmp(agenda[i].correo, correo) == 0) {
            *indiceExistente = i;
            return true;
        }
    }
...
void agregarContacto() {
    ...
    while (1) {
        ...
        if (correoYaExiste(agenda[totalContactos].correo, &indiceExistente)) {
            gotoxy(15, 7);
            printf("Este correo es de %s. Desea sobrescribirlo? (s/n): ", agenda[indiceExistente].nombre);
            char opcion = getch();
            if (opcion == 's' || opcion == 'S') {
                strcpy(agenda[indiceExistente].nombre, agenda[totalContactos].nombre);
                gotoxy(15, 9);
